...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// Создаем представление для отображения OpenGL
	//m_pOpenGLView = (COpenglView*)(RUNTIME_CLASS(COpenglView))->CreateObject();

	//if (!m_pOpenGLView->Create(NULL, NULL, AFX_WS_DEFAULT_VIEW,
	//	CRect(0,0,0,0), this, AFX_IDW_PANE_FIRST+1))
	//{
	//	TRACE(traceAppMsg, 0, "Warning: could not create view for frame.\n");
	//	return -1;        // can't continue without a view
	//}
	//CCreateContext* pContext = (CCreateContext*)lpCreateStruct->lpCreateParams;
	//pContext->m_pCurrentDoc->AddView(m_pOpenGLView);

	//int ScreenSizeX = ::GetDeviceCaps(::GetDC(NULL), LOGPIXELSX);
	//int ScreenSizeY = ::GetDeviceCaps(::GetDC(NULL), LOGPIXELSY);
	//
	//m_pOpenGLView->SetWindowPos(NULL, ScreenSizeX-641, 0, 640, 480, SWP_SHOWWINDOW);
